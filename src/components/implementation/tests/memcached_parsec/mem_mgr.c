...
static struct cos_compinfo men_mgr_info, client_info[NUM_NODE], mc_info;
...
mem_mgr_init(vaddr_t untype, int size, vaddr_t vas_start)
{
	...
	cos_compinfo_init(&men_mgr_info, MEM_SELF_PT, MEM_SELF_CT, 0, vas_start, MEM_CAPTBL_FREE, &men_mgr_info);
	cos_compinfo_init(&mc_info, MEM_COMP_PT_BASE, 0, 0, vas_start, 0, &men_mgr_info);
...
alias_pages(int node, void *addr, int size)
{
	int i;
	vaddr_t dst, start;
	struct cos_compinfo *info;

	if (node < NUM_NODE/2) info = &mc_info;
	else info = &client_info[node];

	start = cos_mem_alias(info, &men_mgr_info, (vaddr_t)addr);
	for(i=1; i<size; i++) {
		dst = cos_mem_alias(info, &men_mgr_info, (vaddr_t)addr+i*PAGE_SIZE);
		assert(dst == start + i*PAGE_SIZE);
	}
	return (void *)start;
}
