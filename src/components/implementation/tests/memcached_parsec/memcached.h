...
int mc_set_key_ext(int node, char *key, int nkey, char *data, int nbytes);
...
void mc_init(int node, vaddr_t untype, int size);
void mc_server_start(int cur);
void mc_print_status(void);
void mc_hashtbl_flush(int cur);
void mc_mem_balance(int cur);
...
DECLARE_INTERFACE(mc_mem_balance)
