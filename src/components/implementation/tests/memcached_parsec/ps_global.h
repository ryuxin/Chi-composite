...
/* Memory header */
struct ps_mheader {
	slab_type_t       type;    /* 0-in use 1-free 2-quisce */
...
static inline void
__ps_mhead_setfree(struct ps_mheader *h, slab_type_t token)
{
	/* TODO: atomic w/ error out */
	h->type = token; /* Assumption: token must be guaranteed to be non-zero */
}
...
struct ps_slab_info {
	...
	struct ps_slab_freelist el;	      /* list of slabs with no available objects */
