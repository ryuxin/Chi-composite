...
struct ps_slab {
	...
	size_t             nquise;   /* # quisce in freelist */
} PS_PACKED;
...
static inline void
__ps_qsc_enqueue(struct ps_qsc_list *ql, struct ps_slab *n)
{
	struct ps_slab *t;

	t = ql->tail;
	if (likely(t)) t->list.next  = ql->tail = n;
	else           ql->head = ql->tail = n;
#ifdef NON_CC_OP
	if (likely(t)) cos_wb_cache(&t->list.next);
	cos_wb_cache(ql);
#endif
}
...
{
	struct ps_slab *a = ql->head;

	if (a) {
		ql->head = (struct ps_slab *)(a->list.next);
...
#else
	__ps_mhead_setfree(h, SLAB_QUIESCE);
	/* do not give back to freelist */
	s->nquise++;
	if (s->nquise == max_nobjs) {
		assert(s->nfree == 0);
#endif
